...
const canWeDeliver = (zipCode) => {
    for (let i =0; i < deliveryAreaZipCodes.length; i++) {
        ...
        if (zipCode === deliveryAreaZipCodes[i]) {
            return `you're in -- we deliver to YOU`
            
        }

    }
    return `sorry please come back when you've relocated to one of the following zip codes: ${deliveryAreaZipCodes}`
}
...
const canWeDeliver2 = (zipCode) => {
    let weDeliver = deliveryAreaZipCodes.includes(zipCode) 
    if (weDeliver) {
        return `we deliver`
    }
    // deliveryAreaZipCodes[i].includes(zipCode) 
    return `Sorry, no pizza for you, buddy!`
    
}
console.log(canWeDeliver2(0000))
...
deals[0].title = deals[0].title.replace(`15`,`10`) //takes 2 arguments, the 'pattern' or the item you would like to replce with the 'replacement' or the new value
